    ...
    <main>
      ...
      <div class="home-about">
        ...
        while($item = mysqli_fetch_assoc($statement)) {
          ...
          $statement2 = mysqli_query($db, "SELECT * FROM post_comments WHERE idPost = ".$item['idPost']."");
          $total=mysqli_num_rows($statement2);
          ...
          <div class="main-section">
            ...
            echo '<div id="seemore-'.$item['idPost'].'" class="row">
...
if(isset($_GET['COMMENT']) && $item['idPost']==$post)
 ...
 if(!isset($_GET['all']) || $item['idPost']!=$post)
 {
          echo '<div  class="sub-comment">';
          while($comment = mysqli_fetch_assoc($statement2)) {
            $path="discussion.php?comment=".$comment['idComment']."&idLivre=".$item['idLivre']."";
            ...
            echo '<div id="redirect-'.$item['idLivre'].'" class="user-detail">';
            ...
            echo'        </div>
                  ...
                  $i++;
                  $target="accueil.php?COMMENT=".$i."&POST=".$comment['idPost']."#seemore-".$item['idPost']."";
                  $target2="accueil.php?POST=".$comment['idPost']."&all='1'#seeall-".$item['idPost']."";
                  $target3="accueil.php?COMMENT=0&POST=".$comment['idPost']."#seemore-".$item['idPost']."";
                 
                  if($i%3==0) 
                  {
                    if($i!=$total){
                      
                  echo '<a href="'.$target.'"  >voir plus</a>';
                  echo '<span style="color:gray;"> / </span>';
                  echo '<a href="'.$target2.'">voir tout</a>';
             
                    }
                  }
                  else {
                    if($i==$total && $i>=3){
                      echo '<a href="'.$target3.'">revenir</a>';
                    }
                  }
          ...
          if( $item['idPost']==$post)
          {
            ...
            $statement2 = mysqli_query($db, "SELECT * FROM post_comments WHERE idPost = ".$item['idPost']."");
            $total=mysqli_num_rows($statement2);
            echo '<div id="seeall-'.$item['idPost'].'" class="sub-comment">';
            while($comment = mysqli_fetch_assoc($statement2)) {
              $path="discussion.php?comment=".$comment['idComment']."&idLivre=".$item['idLivre']."";
              echo '<div class="example">
                      <div class="comment-img"><img src="https://upload.wikimedia.org/wikipedia/commons/7/70/User_icon_BLACK-01.png" class="example"></div>
         ...
         echo ' <div class="comment-box box">              
                <div class="comment-btn">
                  <input  type="textarea" class="text" rows="10" cols="30" placeholder="Donner un avis..."autofocus>
                </div>
              </div>
      </div>
